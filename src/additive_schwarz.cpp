...
AdditiveSchwarzPrecond::AdditiveSchwarzPrecond(const CSRMatrix &A, int overlap_size, MPI_Comm mpi_comm)
    : overlap(overlap_size), factorized(false), comm(mpi_comm) {
    
    local_size = A.nrows;
    
    // For simplicity, extract local subdomain as dense matrix
    // In production code, keep it sparse and use sparse direct solver
    ...
    factorize_local_subdomain(A);
}

void AdditiveSchwarzPrecond::factorize_local_subdomain(const CSRMatrix &A) {
    // Simple LU factorization with partial pivoting (LAPACK-style)
    // For production, use LAPACK or sparse direct solvers
    ...
    for (int k = 0; k < n; ++k) {
        ...
        if (max_val < 1e-14) {
            // Singular matrix, add small diagonal perturbation
...
void AdditiveSchwarzPrecond::solve_local(const std::vector<double> &rhs, std::vector<double> &solution) {
    if (!factorized) {
        solution = rhs;
        return;
    }
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < i; ++j) {
            solution[i] -= local_A_inv[i * n + j] * solution[j];
        }
    }
